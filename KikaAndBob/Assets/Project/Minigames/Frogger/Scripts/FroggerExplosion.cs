...
public class FroggerExplosion : MonoBehaviour 
{
	...
	// Return the blast size in world coordinates
	public Vector2 BlastSizeWorld
	{
		get
		{
			return new Vector2(blastSize.x * transform.localScale.x,
				blastSize.y * transform.localScale.y);
		}

		set
		{
			blastSize = new Vector2(value.x / transform.localScale.x,
				blastSize.y / transform.localScale.y);
		}
	}

	// Return the blast size in local coordinates
	public Vector2 BlastSizeLocal
	{
		get
		{
			return blastSize;
	...
	private IEnumerator ExplodeRoutine()
	{
		...
		float xOffset = blastSize.x * transform.localScale.x * 0.5f;
		float yOffset = blastSize.y * transform.localScale.y * 0.5f;
